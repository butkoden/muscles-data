...
def test_default_catalog_keeps_vendor_adapters_external():
    catalog = DataAdapterCatalog.with_defaults()

    for resource_type in ("elasticsearch", "opensearch", "qdrant", "redis", "sqlalchemy", "mongodb", "s3"):
        assert not catalog.has_factory(resource_type)

    for resource_type in ("memory_vector", "memory_search", "memory_object", "memory_kv", "memory_document", "sql"):
        assert catalog.has_factory(resource_type)
